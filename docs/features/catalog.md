...
| Experimental CDP bridge launch / restart / connect | Experimental | [`cdp/`](../../src-tauri/src/cdp), `launch_codex_with_cdp`, `restart_codex_with_cdp`, `connect_existing_codex_cdp`, `retry_cdp_bridge`, and shared [`SettingsPanel.svelte`](../../ui/SettingsPanel.svelte) | Users may quit then launch the unmodified official App, explicitly restart exactly one normal App after a warning confirmation, or connect an already loopback-CDP-enabled App without restarting it. Restart rejects zero, multiple, stale, tracked, or already-CDP App candidates; it revalidates the exact same-user official PID before one graceful `SIGTERM`, waits boundedly, and never force-kills or falls back to another App. Automatic Connect accepts one same-user official App candidate; custom Connect validates only the selected loopback port. CoPets tracks the exact PID/listener, rejects helper-only or arbitrary endpoints, and rechecks before every send. One native hard deadline bounds listener/DevTools readiness; existing-endpoint Connect/Retry use short bounded attempts within it, accept bounded HTTP `Content-Length` replies without waiting for a socket close, and start external listener monitoring only after Ready. Main-window-preferred concurrent probes avoid cold overlay head-of-line blocking. Retry rechecks only the same native endpoint; runtime port/PID/target/payload stay native-memory only |
